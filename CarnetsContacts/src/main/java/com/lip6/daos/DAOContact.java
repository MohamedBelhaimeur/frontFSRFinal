...
public class DAOContact implements IDAOContact {
	...
	public boolean isDetached(Contact entity) {
	    EntityManager em=JpaUtil.getEmf().createEntityManager();
		 
			// 2 : Ouverture transaction 
			EntityTransaction tx =  em.getTransaction();
	    boolean ok=  entity.getIdContact() !=null // must not be transient
	        && !em.contains(entity)  // must not be managed now
	        && em.find(Contact.class, entity.getIdContact()) != null;  
	    tx.commit();
	    em.close();
	    return ok;// must not have been removed
	}
	/**
	...
	/*	String requete="Select c FROM Contact c JOIN FETCH c.phones WHERE c.idContact=:id" ;
		...
		*/
		...
		c.getPhones().forEach(x->System.out.println(x.getPhoneKind()));
		tx.commit();
		em.close();
	...
	@Override
	public boolean modifyContact(Contact c,Long id) {
		boolean success = false;
		...
		try {
			 ...
			 EntityManager em=JpaUtil.getEmf().createEntityManager();
				...
				Contact toUpdate=em.find(Contact.class, id);
				
				toUpdate.setFirstName(c.getFirstName());
				toUpdate.setLastName(c.getLastName());
				toUpdate.setEmail(c.getEmail());
				
			Address ad=	em.find(Address.class, toUpdate.getAddress().getId());
			ad.setCity(c.getAddress().getCity());
			ad.setCountry(c.getAddress().getCountry());
			ad.setZip(c.getAddress().getZip());
			ad.setStreet(c.getAddress().getStreet());

				toUpdate.setContactGroups(c.getContactGroups());
